...
if tab == 0:
    ...
    if not df_eng.empty:
        ...
        # ── Pendo engagement trend charts ───────────────────────────────────────
        try:
            import datetime as _dt
            from utils.pendo_conn import get_page_views, get_combined_page_views
            _pids_trend = customer.get("pendo_ids", [customer.get("pendo_id", customer_name.lower())])

            with st.spinner("Loading engagement trends..."):
                if len(_pids_trend) > 1:
                    _dp_trend = get_combined_page_views(_pids_trend, 180)
                else:
                    _dp_trend = get_page_views(_pids_trend[0], 180)

            if not _dp_trend.empty and "date" in _dp_trend.columns:
                import pandas as _pd2
                _dp_trend["date"] = __import__("pandas").to_datetime(_dp_trend["date"])
                _dp_trend["week"] = _dp_trend["date"].dt.to_period("W").dt.start_time

                weekly = _dp_trend.groupby("week").agg(
                    total_minutes=("numMinutes", "sum"),
                    total_events =("numEvents",  "sum"),
                    unique_users =("visitorId",  "nunique")
                ).reset_index()
                weekly["week_str"] = weekly["week"].dt.strftime("%b %d")
                weekly["total_hours"] = (weekly["total_minutes"] / 60).round(1)

                st.markdown("#### 📈 Platform engagement trends (last 6 months · weekly)")
                st.caption("Source: Pendo · Time on platform and views/clicks rolled up weekly · "
                           "Mirrors Pendo's own analytics view")
                ...
                with tr1:
                    ...
                    fig1.add_trace(go.Scatter(
                        x=weekly["week_str"],
                        y=weekly["total_hours"],
                        mode="lines+markers",
                        name="Hours on platform",
                    ...
                    fig1.update_layout(
                        title=dict(text="⏱ Time on platform (hrs/week)",
                ...
                with tr2:
                    ...
                    fig2.update_layout(
                        title=dict(text="👆 Views & clicks (events/week)",
                                   font=dict(size=13, color="#e6edf3")),
                        paper_bgcolor="rgba(0,0,0,0)",
                        plot_bgcolor="rgba(0,0,0,0)",
                        font=dict(color="#8b949e", size=10),
                ...
                fig3.add_trace(go.Bar(
                    x=weekly["week_str"],
                    y=weekly["unique_users"],
                    name="Active users",
                    marker_color="#9B59B6",
                    opacity=0.8,
                ))
                fig3.update_layout(
                    title=dict(text="👥 Weekly active users",
                               font=dict(size=13, color="#e6edf3")),
                    paper_bgcolor="rgba(0,0,0,0)",
                    plot_bgcolor="rgba(0,0,0,0)",
                    font=dict(color="#8b949e", size=10),
        ...
        except Exception as _pendo_err:
