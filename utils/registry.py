"""
queries/registry.py — Central SQL registry. Add new queries here, reference in pages.
<SCHEMA> is replaced at runtime with the customer schema name.
"""
...
QUERIES = {
    ...
    "frt_monthly": """
        ...
        WHERE sl_created_at >= '2025-06-01' AND sl_is_bot = 'false'
    ...
    "escalation_monthly": """
        WITH t AS (
            ...
            FROM PIPE_DATABASE.<SCHEMA>.case_summary
            WHERE sl_created_at >= '2025-06-01' AND sl_created_at < CURRENT_DATE()
            GROUP BY 1
        )
    ...
    "ica_events_monthly": """
        SELECT TO_CHAR(DATE_TRUNC('MONTH', event_time),'YYYY-MM') AS month,
            SUM(CASE WHEN event_source = 'ICA_AUTO_ASSIGNMENT'   THEN 1 ELSE 0 END) AS auto_assigned,
            SUM(CASE WHEN event_source = 'ICA_MANUAL_ASSIGNMENT' THEN 1 ELSE 0 END) AS manual_assigned,
    ...
    "ica_total_lifetime": """
        SELECT
            SUM(CASE WHEN event_source = 'ICA_AUTO_ASSIGNMENT'   THEN 1 ELSE 0 END) AS total_auto,
            SUM(CASE WHEN event_source = 'ICA_MANUAL_ASSIGNMENT' THEN 1 ELSE 0 END) AS total_manual
        FROM PIPE_DATABASE.<SCHEMA>.std_event_log
    ...
    "lte_accuracy_monthly": """
        SELECT TO_CHAR(DATE_TRUNC('MONTH', ep.ts),'YYYY-MM') AS month,
            ...
            COUNT(DISTINCT CASE WHEN cs.sl_escalated_at_first IS NOT NULL THEN ep.sl_case_id END) AS actually_escalated,
            ROUND(100.0 * COUNT(DISTINCT CASE WHEN cs.sl_escalated_at_first IS NOT NULL THEN ep.sl_case_id END)
    ...
    # ── Platform engagement (agent actions on SL) ─────────────────────────────
    ...
    # ── SL Feedback (agent labeling signals) ─────────────────────────────────
